...
//	String version = "2023.1.2";
	...
	private class Converter {
		...
		public void convertPerceptaResultsToPredictionDashboard(List<Model> models, List<PredictionDashboard> allResults,
				APIMolecule ac, Hashtable<String, DsstoxRecord> htDTXCIDtoDsstoxRecord,
				Hashtable<String, Dataset> htModelNameToDataset,HashSet<String>cidsLoaded, boolean createReports, boolean saveReportsToHarddrive) {
...
//			String smilesSDF=ac.getProperty("smiles");
			...
			if(dtxcid==null) return;
		
			
			if(cidsLoaded.contains(dtxcid)) {
				return;
			}
...
//			boolean printedSmilesComparison=false;
			...
			String errorStructure=null;
						
			if (ac.getProperty("SMILES")!=null) {
				if (ac.getProperty("SMILES").contains(".")) 
					isSalt=true;
			} else {
				try {
					if (StructureUtil.isSalt(StructureUtil.fromMolString(ac.strStructure))) 				
						isSalt=true;
				} catch (Exception ex) {
					System.out.println("Couldnt parse molecule"+ac.getProperty("DTXCID"));
					errorStructure="Couldn't parse mol file";
				}
			}
			...
			for (Model model : models) {
				...
				if(caution!=null) {
					...
					if (isSalt) {
//						pd.setPredictionError("Invalid structure: multiple molecules");
//						pr.modelResults.predictedError=pd.getPredictionError();
						System.out.println("Is salt but have no caution:"+dtxcid);
					}			
					...
					if (isSalt) {
						...
						pd.setPredictionError("Incorrect structure: Molecule contains too many fragments");
					}
...
//				}
		...
		private int get_pKa_Num(APIMolecule ac, String propertyName) {
		...
		private void setExperimentalConditions(APIMolecule ac, String modelName, Property property, Percepta_Report pr) {
			if (property.getName().equals(DevQsarConstants.BOILING_POINT)) {
				pr.modelResults.pressure = Double.parseDouble(ac.getProperty(modelName + "_Press"));
				pr.modelResults.pressureUnits = ac.getProperty(modelName + "_PressUnits");
			}
		...
		private void setPKA(gov.epa.run_from_java.scripts.PredictionDashboard.PredictionReport pr, APIMolecule ac, PredictionDashboard pd, int num) {
			pr.modelResults.predictedValue = Double.parseDouble(ac.getProperty("ACD_pKa_Apparent_" + num));
			pr.modelResults.predictedUncertainty = Double.parseDouble(ac.getProperty("ACD_pKa_Error_Apparent_" + num));
			pd.setPredictionValue(pr.modelResults.predictedValue);
			pr.modelResults.equation = (String) ac.getProperty("ACD_pKa_Equation_Apparent_" + num);
	...
	private class UtilityMethods {
		...
		private  void findMissingValues() {
			...
			String json=JsonUtilities.gson.toJson(missingRecords);
		...
		void printValues(String filepathSDF, List<String> dtxsids) {
			...
			try {
				...
				while (iterator.hasNext()) {
					...
					System.out.println(JsonUtilities.gson.toJson(ac.getProperties()));
				}
...
//			System.out.println(sourceName+"\t"+source==null);
	...
	public void compileResultsFromSDFsInFolder(String folderpath,String snapshotName,boolean omitSalts) {
...
//		System.out.println("Loaded keys: "+loadedKeys.size());
		...
		HashSet<String>cidsLoaded=DatabaseUtilities.getLoadedCIDs(sourceName, ">=15");
		...
		} else if (snapshotName.contentEquals("DSSTOX Snapshot 2025-12-31")) {
			htDTXCIDtoDsstoxRecord = dsstoxRecordService
					.getDsstoxRecordsHashtableFromJsonExport2(PredictionDashboardTableMaps.fileJsonDsstoxRecords2025_12_31, "dtxcid");
		} else {
...
//		Logger mdlV3000ReaderLogger = Logger.getLogger(org.openscience.cdk.io.MDLV3000Reader.class.getName());
//		mdlV3000ReaderLogger.setLevel(Level.OFF);
		...
		for (File file : new File(folderpath).listFiles()) {
			...
			List<PredictionDashboard> pds = getResultsFromSDFFile(file.getAbsolutePath(), models,
					htDTXCIDtoDsstoxRecord, htModelNameToDataset, skipMissingSID, maxCount,omitSalts,cidsLoaded,
					createReports, saveReportsToHarddrive);
			...
			if (writeToDB && pds.size()>0) {
				predictionDashboardService.createBatch(pds);// write to DB using sql
			}
			
			System.out.println("\n"+file.getName()+"\t"+pds.size()+"\t"+cidsLoaded.size());
...
//			}
	...
	void loadPredictions2024() {
		...
		compileResultsFromSDFsInFolder("data\\percepta\\2024-11-12\\",snapshotName,omitSalts);
		compileResultsFromSDFsInFolder("data\\percepta\\2024-10\\",snapshotName,omitSalts);

	}

	
	void loadPredictions2026() {
		this.version="2025.1.4";
		String snapshotName="DSSTOX Snapshot 2025-12-31";
		boolean omitSalts=true;
		
		//Load latest results first (dtxcid has changed for some dtxsids):
		compileResultsFromSDFsInFolder("data\\Percepta2025.1.4\\",snapshotName,omitSalts);


	}
	
	
	void findMarkushInOutputSdfs() {
		
		
//		String folderpath="data\\Percepta2025.1.4\\";
		
		String folderpath="data\\dsstox\\snapshot-2025-12-31\\";
		...
		for (File file : new File(folderpath).listFiles()) {
			...
			List<APIMolecule>acs=StructureUtil.readSDF_to_API_Molecules(file.getAbsolutePath(), -1);
			
			for (APIMolecule ac:acs) {
				
				if(ac.strStructure.contains("ATTACH=ANY")) {
					System.out.println("\t"+ac.getProperty("DTXCID"));
				}
...
//		String folderpath="data\\dsstox\\snapshot-2025-12-31\\";
		...
		List<String>dtxcids=Arrays.asList("DTXCID501610869","DTXCID702047124","DTXCID901629276");
		...
		for (File file : new File(folderpath).listFiles()) {
			...
			for (APIMolecule ac:acs) {
				...
				if(dtxcids.contains(ac.getProperty("DTXCID"))) {
					System.out.println(file.getName()+"\t"+JsonUtilities.gson.toJson(ac.htProperties));
				}
...
//		p.loadPredictions2024();
		p.loadPredictions2026();
		
//		p.findMarkushInOutputSdfs();
//		p.findDtxcidsInOutputSdfs();
...
//		p.utilities.lookAtMissingValues();
	...
	public List<PredictionDashboard> getResultsFromSDFFile(String filepathSDF, List<Model> models,
			...
			boolean skipMissingSID, int maxCount,boolean omitSalts,HashSet<String>cidsLoaded, boolean createReports, boolean saveReportsToHarddrive) {
...
//			Iterator<IAtomContainer> iterator = acs2.atomContainers().iterator();
			...
			//Use custom APIMolecule because couldnt seem to turn off logger for bad dsstox compounds:
			List<APIMolecule>acs=StructureUtil.readSDF_to_API_Molecules(filepathSDF, -1);
			
//			List<APIMolecule> acs2 = dpu.filterAPIMolecules(acs, skipMissingSID, omitSalts, maxCount);
//			Iterator<APIMolecule> iterator = acs2.iterator();
			
			Iterator<APIMolecule> iterator = acs.iterator();
			System.out.println("\nAtom container count=" + acs.size());

			List<PredictionDashboard> allResults = new ArrayList<>();

			while (iterator.hasNext()) {
				APIMolecule ac = iterator.next();
				converter.convertPerceptaResultsToPredictionDashboard(models, allResults, ac, htDTXCIDtoDsstoxRecord,
						htModelNameToDataset,cidsLoaded,createReports, saveReportsToHarddrive);
			}
