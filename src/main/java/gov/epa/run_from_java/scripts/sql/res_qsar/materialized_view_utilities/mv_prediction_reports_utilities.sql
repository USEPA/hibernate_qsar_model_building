...
-- SELECT * FROM event_log(TABLE(<fully-qualified-table-name>)) WHERE event_type = "update_progress" ORDER BY timestamp desc;
...
select count(distinct(dtxsid)), source_name from public.mv_predicted_reports
...
UPDATE qsar_models.prediction_reports
SET
  file_json = convert_to(
    replace(
      convert_from(file_json, 'UTF8'),
      'https://ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/',
      'https://comptox.epa.gov/ctx-api/chemical/property/model/file/search/'
    ),
    'UTF8'
  ),
  file_html = convert_to(
    replace(
      convert_from(file_html, 'UTF8'),
      'https://ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/',
      'https://comptox.epa.gov/ctx-api/chemical/property/model/file/search/'
    ),
    'UTF8'
...
-- look at values
select convert_from(pr.file_json, 'UTF8'), convert_from(pr.file_html, 'UTF8')
FROM qsar_models.prediction_reports pr
WHERE pr.id = 106325629;

-- fix reports for ctx-api link:
UPDATE qsar_models.prediction_reports
SET
  file_json = convert_to(
    replace(
...
WHERE
  convert_from(file_json, 'UTF8') LIKE '%ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/%'
  OR convert_from(file_html, 'UTF8') LIKE '%ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/%';


--Update one row:
UPDATE qsar_models.prediction_reports AS pr
SET
  file_json = x.new_json_b,
  file_html = x.new_html_b
FROM (
  SELECT
    id,
    convert_to(
  ...
  FROM qsar_models.prediction_reports
  WHERE id = 106325629
) AS x
WHERE pr.id = x.id
  AND (pr.file_json IS DISTINCT FROM x.new_json_b
       OR pr.file_html IS DISTINCT FROM x.new_html_b);
...
FROM (
  SELECT
    id,
    convert_to(
      replace(convert_from(file_json, 'UTF8'),
              'https://ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/',
              'https://comptox.epa.gov/ctx-api/chemical/property/model/file/search/'),
      'UTF8'
    ) AS new_json_b,
    convert_to(
      replace(convert_from(file_html, 'UTF8'),
              'https://ctx-api-dev.ccte.epa.gov/chemical/property/model/file/search/',
              'https://comptox.epa.gov/ctx-api/chemical/property/model/file/search/'),
      'UTF8'
    ) AS new_html_b
