...
select distinct  source from skin_eye

# select count(toxval_id) from toxval t where t.source = 'NLM ChemIDPlus' and toxval_type='LD50' and species_original='rat' and exposure_route='oral'


select t.source, count(distinct(t.dtxsid)) from toxval t
where toxval_type='LD50' and species_original='rat' and exposure_route='oral'
group by t.source;
