...
//					er=Utilities.gson.fromJson(json, EpisuiteResults.class);
					jo=JsonUtilities.gson.fromJson(json, JsonObject.class);
...
//			if(true)return;
	...
	public void runSmilesFilePublicApiUsingDpcSmiles(String propertyName,String modelName,  Hashtable<String,DataPoint>htDP, String filepathOut) {
...
//			if(true)return;
			...
			for (String qsarSmiles:htDP.keySet()) {
				...
				DataPoint dp=htDP.get(qsarSmiles);
				CheckStructure cs=dp.checkStructure;
				
				String smiles=cs.dpc_smiles;
				...
				try {
					...
					jo.addProperty("canonQsarSmiles", qsarSmiles);
					jo.addProperty("smiles", smiles);
					System.out.println(counter+"\t"+cs.dpc_dtxcid+"\t"+json+"\t"+qsarSmiles);
					fw.write(gson.toJson(jo)+"\r\n");
				}
			}
			fw.close();
