...
public class SplittingGeneratorPFAS_Script {
	...
	public static final String splitting_30_percent_F = "T=30% F, P=PFAS";
	public static final String splitting_3F = "T=3F, P=PFAS";
	public static final String splitting_2F = "T=2F, P=PFAS";
	public static final String splitting_1F = "T=1F, P=PFAS";
...
//				System.out.println(dr.dsstoxCompoundId+"\t"+dr.smiles+"\t"+dr.qsarReadySmiles);
	...
	/**
	 * Creating a local copy of one of Gabriel's methods because i needed to turn of
	 * refresh to get it to not hang up after 1 chemical
	 * 
	 * @param dpis
	 * @return
...
//			session.refresh(dpis);//need to turn this off or it hangs!
	...
	private String standardize(DsstoxRecord dr, SciDataExpertsStandardizer standardizer, int counter) {
		...
		Compound compound = compoundService.findByDtxcidSmilesAndStandardizer(dr.dsstoxCompoundId, dr.smiles,
				standardizer.standardizerName);
		...
		if (compound != null) {
			...
			System.out.println(counter + "\t" + dr.dsstoxCompoundId + "\tAlready have qsar ready smiles="
					+ compound.getCanonQsarSmiles());
			...
			if (standardizeResult.status == 200) {
				...
				compound = new Compound(dr.dsstoxCompoundId, dr.smiles, standardizeResult.qsarReadySmiles,
						standardizer.standardizerName, "tmarti02");
				System.out.println(counter + "\t" + dr.dsstoxCompoundId + "\tSDE qsar ready smiles="
						+ compound.getCanonQsarSmiles());
				...
				System.out.println(": Standardizer HTTP response failed for SMILES: " + dr.smiles + " with code "
						+ standardizeResult.status);
	...
	void createSplitting(String datasetName, String splittingName, HashSet<String> smilesArray) {
		...
		List<DataPointInSplitting> dataPointsInSplitting = dataPointInSplittingService
				.findByDatasetNameAndSplittingName(datasetName, "RND_REPRESENTATIVE");
		...
		for (DataPointInSplitting dpis : dataPointsInSplitting) {
			DataPointInSplitting dpisNew = new DataPointInSplitting(dpis.getDataPoint(), splitting, dpis.getSplitNum(),
					lanid);
			...
			// Only add ones that are in training and test set (ignore ones with
			// SplitNum=2):
...
//			dataPointInSplittingService.create(dpisNew);
	...
	void createFiveFoldExternalSplittings(String folder, String datasetName, String descriptorSet,
			ArrayList<String> smilesArrayPFAS) {
		...
		try {
			...
			for (String Line : Lines) {
				...
				if (smilesArrayPFAS.contains(smiles))
					linesPFAS.add(Line);
				else
					linesNonPFAS.add(Line);
//				System.out.println(smiles);
			...
			for (int i = 0; i <= 4; i++) {
				FileWriter fwPred = new FileWriter(
						folder + datasetName + "_" + descriptorSet + "_prediction_PFAS" + (i + 1) + ".tsv");
				FileWriter fwTrainPFASOnly = new FileWriter(
						folder + datasetName + "_" + descriptorSet + "_training_PFAS" + (i + 1) + ".tsv");
				FileWriter fwTrainAll = new FileWriter(
						folder + datasetName + "_" + descriptorSet + "_training_All" + (i + 1) + ".tsv");
				FileWriter fwTrainAllButPFAS = new FileWriter(
						folder + datasetName + "_" + descriptorSet + "_training_All_but_PFAS" + (i + 1) + ".tsv");
	...
	void getPFASChemicalCountForDataSet(String datasetName, ArrayList<String> smilesArray) {
		...
		for (DataPoint dp : dataPoints) {
			if (smilesArray.contains(dp.getCanonQsarSmiles()))
				count++;
		}
	...
	public static int getCount(Connection conn, String datasetName, String splitting, int splitNum) {
		...
		String sql = "select count(dp.id) from qsar_datasets.data_points dp\n"
				...
				+ datasetName.replace("'", "''") + "'\n" + "and s.\"name\"='" + splitting + "' and dpis.split_num = "
				+ splitNum + ";";
...
//		System.out.println(sql+"\n");
	...
	void createSpecialFluorineSplittings() {
...
//		splittingNames.add(splitting_30_percent_F);
//		splittingNames.add(splitting_3F);
		splittingNames.add(splitting_2F);
		splittingNames.add(splitting_1F);
...
//		p.generateQSAR_ReadyPFAS_STRUCT(listName,filePath);		
		...
		System.out.println("datasetName\tcountPFAS\tcountFractionF\tcountWith1F\tcountWith2F\tcountWith3F");

		for (String datasetName : datasetNames) {
			for (String splittingName : splittingNames) {
				createSplittingCustom(datasetName, splittingName, smilesArray, writeToDB);
...
//				System.out.println(datasetName+"\t"+splittingName+"\t"+countTraining + "\t" + countPrediction);
	...
	private void createSplittingCustom(String datasetName, String splittingName, HashSet<String> smilesArray,
			boolean writeToDB) {

		String lanid = "tmarti02";

		List<DataPointInSplitting> dataPointsInSplitting = dataPointInSplittingService
				.findByDatasetNameAndSplittingName(datasetName, DevQsarConstants.SPLITTING_RND_REPRESENTATIVE);

		SplittingService splittingService = new SplittingServiceImpl();
		Splitting splitting = splittingService.findByName(splittingName);
		if (splitting == null) {
			System.out.println(splittingName + " is null, creating...");
			splitting = new Splitting(splittingName, splittingName, 2, lanid);
			splitting = splittingService.create(splitting);
			System.out.println(splittingName + " created");
		}
//		System.out.println(splitting.getId() + "\t" + splitting.getDescription());
		...
		ci.descriptorSetName = DevQsarConstants.DESCRIPTOR_SET_WEBTEST;
		ci.splittingName = DevQsarConstants.SPLITTING_RND_REPRESENTATIVE;
		ci.remove_log_p = false;
		ModelData data = ModelData.initModelData(ci, false);

		CSVLoader csvLoader = new CSVLoader();
		Instances iTraining = csvLoader.getDataSetFromString(data.trainingSetInstances);
//		System.out.println(iTraining.numInstances());

		int countPFAS = 0;
		int countFractionF = 0;
		int countWith3F = 0;
		int countWith2F = 0;
		int countWith1F = 0;
...
//			
			if (dpis.getSplitNum() == DevQsarConstants.TRAIN_SPLIT_NUM) {
				...
				String smiles = dpis.getDataPoint().getCanonQsarSmiles();
				Instance instance = iTraining.instance(smiles);

				double numF_aliphatic = instance.value("-F [aliphatic attach]");
//				double numF_olefinic=instance.value("-F [olefinic attach]");
				double numCF3 = instance.value("-CF3 [aliphatic attach]");

//				double num=numF_aliphatic+numF_olefinic+3*numCF3;
				double num = numF_aliphatic + 3 * numCF3;

				double MW = instance.value("MW");
				double frac = 19 * num / MW;

				boolean isPFAS = isPFAS(smilesArray, dpis);
				...
				if (splittingName.equals(splitting_3F) && num < 3)
					dpisNew.setSplitNum(2);
				else if (splittingName.equals(splitting_2F) && num < 2)
					dpisNew.setSplitNum(2);
				else if (splittingName.equals(splitting_1F) && num < 1)
					dpisNew.setSplitNum(2);
				else if (splittingName.equals(splitting_30_percent_F) && frac < minFrac)
					dpisNew.setSplitNum(2);

//				if(isPFAS) {
//					System.out.println(smiles+"\t"+numF);
//				}

//				if(frac>minFrac || isPFAS) {
//					System.out.println(smiles+"\t"+numF+"\t"+frac+"\t"+isPFAS);
//				}
...
//			//Only add ones that are in training and test set (ignore ones with SplitNum=2):
		...
		if (splittingName.equals(splitting_1F)) {

			System.out.println(
					datasetName + "\t" + countPFAS + "\t" + countFractionF + "\t" + countWith1F + "\t"+countWith2F + "\t" + countWith3F);
...
//		datasetNames.add("WS v1 res_qsar");
		...
		for (String datasetName : datasetNames) {
			for (String splittingName : splittingNames) {
				...
				DatasetFileWriter.writeWithSplitting(descriptorSetName, splittingName, datasetName, folder, true,
						false);
			}
		}
...
//		String splittingName = splitting_3F;
		String splittingName=splitting_30_percent_F;
		...
		for (String dataSetName : datasetNames) {
			...
			List<DataPointInSplitting> dpisList = dpisService.findByDatasetNameAndSplittingName(dataSetName,
					splittingName);

			System.out.println(dataSetName+"\t"+dpisList.size());
