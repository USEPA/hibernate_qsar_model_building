...
public class DsstoxMapper implements AutoCloseable {
	...
	public List<MappedPropertyValue> map(List<PropertyValue> propertyValues) {
		...
		if (datasetParams.mappingParams.isNaive) {	
			...
			dsstoxRecords = queryDsstox.getRecords(propertyValuesMap.keySet(), datasetParams.mappingParams.dsstoxMappingId);
		} else {
		...
		List<MappedPropertyValue> mappedPropertyValues = mapPropertyValuesToDsstoxRecordsNew();
	...
	private List<MappedPropertyValue> mapPropertyValuesToDsstoxRecordsNew() {
	...
	private boolean findMappingConflicts(DsstoxConflict conflict, String bin) {
		...
		if (bin.contains(OTHER_CASRN_CONFLICT)) {
			...
			List<DsstoxRecord> otherCasrnConflicts = queryDsstox.getRecords(sourceChemical.getSourceCasrn(), conflictType);
			...
			List<DsstoxRecord> casrnConflicts = queryDsstox.getRecords(sourceChemical.getSourceCasrn(), conflictType);
		...
		if (bin.contains(PREFERRED_NAME_CONFLICT)) {
			...
			List<DsstoxRecord> preferredNameConflicts = 
					queryDsstox.getRecords(sourceChemical.getSourceChemicalName(), conflictType);
		...
		if (hasSynonymConflict(bin)) {
			List<DsstoxRecord> synonymConflicts = queryDsstox.getRecords(sourceChemical.getSourceChemicalName(), DevQsarConstants.INPUT_SYNONYM);
		...
		if (bin.contains(NAME2STRUCTURE_CONFLICT)) {
			...
			List<DsstoxRecord> structureConflicts = queryDsstox.getRecords(result.smiles, DevQsarConstants.INPUT_SMILES);
		...
		if (bin.contains(MAPPED_IDENTIFIER_CONFLICT)) {
			...
			List<DsstoxRecord> mappedIdentifierConflicts = 
					queryDsstox.getRecords(sourceChemical.getSourceChemicalName(), conflictType);
		...
		if (bin.contains(STRUCTURE_CONFLICT)) {
			...
			List<DsstoxRecord> structureConflicts = queryDsstox.getRecords(sourceChemical.getSourceSmiles(), conflictType);
			if (structureConflicts!=null && !structureConflicts.isEmpty()) {
				for (DsstoxRecord dr:structureConflicts) {
					addDsstoxConflictRecord(dr, conflict, conflictDtxsids, conflictType);
				}
	...
	private boolean findDelimiterConflicts(DsstoxConflict conflict) {
		...
		if (sourceCasrnStr!=null && sourceCasrnStr.contains("|")) {
			...
			for (String sourceCasrn:sourceCasrns) {
				List<DsstoxRecord> casrnConflicts = queryDsstox.getRecords(sourceCasrn, conflictType);
				if (casrnConflicts!=null && !casrnConflicts.isEmpty()) {
					for (DsstoxRecord dr:casrnConflicts) {
						addDsstoxConflictRecord(dr, conflict, conflictDtxsids, conflictType);
					}
		...
		if (sourceDtxsidStr!=null && sourceDtxsidStr.contains("|")) {
			...
			List<DsstoxRecord> dtxsidConflicts = queryDsstox.getRecords(new HashSet<String>(Arrays.asList(sourceDtxsids)), 
					conflictType);
